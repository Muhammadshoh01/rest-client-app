    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className={`flex justify-between items-center transition-all duration-300 ${isSticky ? 'h-14' : 'h-16'
                    }`}>
                    <div className="flex items-center">
                        <Link href="/" className="flex items-center group">
                            <div className={`bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold transition-all duration-300 ${isSticky ? 'w-7 h-7 text-sm' : 'w-8 h-8'
                                }`}>
                                R
                            </div>
                            <span className={`ml-2 font-semibold text-gray-900 transition-all duration-300 group-hover:text-blue-600 ${isSticky ? 'text-lg' : 'text-xl'
                                }`}>
                                REST Client
                            </span>
                        </Link>
                    ...
                    <div className="flex items-center space-x-4">
                        <button
                            ...
                            className={`font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-all duration-200 ${isSticky ? 'px-2.5 py-1 text-xs' : 'px-3 py-1 text-sm'
                                }`}
                        >
                            {language}
                        </button>
                        ...
                        {isAuthenticated ? (
                            <div className="flex items-center space-x-3">
                                {username && (
                                    <span className={`text-gray-600 transition-all duration-300 ${isSticky ? 'text-xs hidden sm:inline' : 'text-sm'
                                        }`}>
                                        {username}
                                    </span>
                                )}
