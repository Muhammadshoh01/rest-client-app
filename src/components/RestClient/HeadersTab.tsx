    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <span className="text-sm font-medium text-gray-700">
          {t('requestHeaders') || 'Request Headers'}
        </span>
        <button
          onClick={onAddHeader}
      ...
      <div className="space-y-3">
        {headers.map((header) => (
          ...
          >
            ...
            <input
              type="text"
              placeholder={t('headerKeyPlaceholder') || 'Header Key'}
              value={header.key ?? ''}
            ...
            <input
              type="text"
              placeholder={t('headerValuePlaceholder') || 'Header Value'}
              value={header.value ?? ''}
              onChange={(e) =>
            ...
            <button
              ...
              title={t('removeHeader') || 'Remove Header'}
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      {headers.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          <p>{t('noHeadersYet') || 'No headers added yet'}</p>
          <button
            onClick={onAddHeader}
            className="mt-2 text-indigo-600 hover:text-indigo-700"
          >
            {t('addFirstHeader') || 'Add your first header'}
          </button>
        </div>
      )}
    </div>
