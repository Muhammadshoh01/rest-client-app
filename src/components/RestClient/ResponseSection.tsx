      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 text-sm font-mono max-h-64 overflow-y-auto">
              {Object.keys(response.headers).length > 0 ? (
                ...
                <div className="text-gray-500">{t('noHeaders')}</div>
              )}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium text-gray-700">{t('body')}</h3>
              {response.body && (
                <button
            ...
            <div className="bg-gray-50 rounded-lg p-4 text-sm font-mono max-h-64 overflow-auto">
              {response.body ? (
                ...
                <div className="text-gray-500">{t('noBody')}</div>
              )}
            </div>
          </div>
        </div>
        ...
        {response.body && (
          <div className="mt-4 text-xs text-gray-500 border-t pt-4">
            {t('responseSize', { size: new Blob([response.body]).size })}
          </div>
        )}
      </div>
    </div>
